 ...
 */
import { Loader2, Swords, Check, RefreshCw } from 'lucide-react';
...
export default function BattlePanel({ campaign, players, myPlayer, mapDef, onPhaseChanged }) {
  ...
  const { cards, loading, reload } = useBattleCards({
    ...
    <div className="p-4 space-y-4 h-full overflow-y-auto dock-scroll">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="px-3 py-2 rounded border border-border bg-muted/20 text-center">
          ...
          <p className="font-mono font-bold text-lg text-foreground">{cards.length}</p>
        </div>
        <div className={`px-3 py-2 rounded border text-center ${resolvedCount === cards.length && cards.length > 0 ? 'border-status-locked/40 bg-status-locked/10' : 'border-border bg-muted/20'}`}>
          <p className="text-muted-foreground">Resolved</p>
          <p className={`font-mono font-bold text-lg ${resolvedCount === cards.length && cards.length > 0 ? 'text-status-locked' : 'text-foreground'}`}>{resolvedCount}/{cards.length}</p>
        </div>
      </div>
