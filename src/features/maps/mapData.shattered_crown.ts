...
const POLYS: Record<string, {x:number,y:number}[]> = {
  ...
  C7: (() => {
    const d = [[7138.67,6494.22],[-4.44,-14.22],[-238.22,-242.67],[-238.22,-242.67],[5.33,-115.56],[72.0,-82.67],[13.33,-98.67],[40.0,13.33],[3.56,63.11],[154.67,5.33],[17.78,-71.11],[77.33,21.33],[77.33,190.22],[164.44,16.89],[130.67,-110.22],[97.78,-242.67]];
    let cx=d[0][0], cy=d[0][1]; const pts=[{x:cx,y:cy}];
    for(let i=1;i<d.length;i++){cx+=d[i][0];cy+=d[i][1];pts.push({x:+cx.toFixed(2),y:+cy.toFixed(2)});}
    return pts;
  })(),
  C8: (() => {
    const d = [[7428.44,5920.89],[2.67,-16.0],[29.33,-223.11],[29.33,-223.11],[167.11,-204.44],[91.56,136.0],[144.89,-16.89],[123.56,229.33],[-38.22,193.78],[-251.56,45.33],[-114.67,-22.22],[-152.0,-137.78]];
    let cx=d[0][0], cy=d[0][1]; const pts=[{x:cx,y:cy}];
    for(let i=1;i<d.length;i++){cx+=d[i][0];cy+=d[i][1];pts.push({x:+cx.toFixed(2),y:+cy.toFixed(2)});}
    return pts;
  })(),
