...
Deno.serve(async (req) => {
  ...
  if (action === 'closeBattleVoting') {
    ...
    if (allAutoResolve && !hasForfeit) {
      ...
      const troopSources = await getBattleTroopSources(base44, campaign_id, freshCard.round ?? round, freshCard);
      const authCard = {
        ...freshCard,
        defender_troops: troopSources.authoritative_defender_troops,
      ...
      await applyAutoResolve(authCard, { ...autoResult, combat_source_trace: troopSources.combat_source_trace }, base44, campaign_id, round);
      ...
      if (card.battle_type === 'double_siege') {
        ...
        if (defenderForfeited) {
          ...
          if (remAutoResolve) {
            ...
            await ensureBattlePhaseStartSnapshot(base44, campaign_id, round, players);
            const convTroopSources = await getBattleTroopSources(base44, campaign_id, freshConv.round ?? round, freshConv);
            const convAuthCard = {
              ...freshConv,
              ...
              combat_source_trace: convTroopSources.combat_source_trace,
            };
            const autoResult = autoResolveBattle(convAuthCard, campaign_id);
            await applyAutoResolve(convAuthCard, { ...autoResult, combat_source_trace: convTroopSources.combat_source_trace }, base44, campaign_id, round);
